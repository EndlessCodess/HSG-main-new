...
void test_nsg()
{
    auto test_result = std::ofstream(std::format("result/nsg/{0}.txt", name), std::ios::app | std::ios::out);
    ...
    test_result << std::format("total hit: {1:<10} average time: {2:<10}us", total_hit, total_time / test.size())
                << std::endl;
    delete index;
    done_semaphore.acquire();
    ++done_mission;

    if (done_mission == done_number)
    {
        done.release();
    }
